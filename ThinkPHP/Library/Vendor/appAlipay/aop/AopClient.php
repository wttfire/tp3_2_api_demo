...
require_once ALIPAY_PATH.'lotusphp_runtime/Logger/Logger.php';
class AopClient {
	//应用ID
	public $appId;
	//私钥文件路径
	...
	protected function logCommunicationError($apiName, $requestUrl, $errorCode, $responseTxt) {
		...
		$logger = new LtLogger;
		$logger->conf["log_file"] = rtrim(ALIPAY_PATH, '\\/') . '/' . "logs/aop_comm_err_" . $this->appId . "_" . date("Y-m-d") . ".log";
	...
	*/
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		$request->setAppSign($requestUrl);
		
		//发起HTTP请求
		 try {
		 	
			$resp = $this->curl($this->gatewayUrl . "?" . $requestUrl, $apiParams);
		} catch (Exception $e) {
